...
@app.get("/preco_ovos/")
def preco_ovos(preco: float,qtd: int):
...
def preco_papel_hig(preco: float,qtd: int, metros:int):
...
def preco_desconto(preco_original: float,preco_com_desconto: float ):
...
def comparar_produtos(preco1: float, qtd1: float, preco2: float, qtd2: float):
